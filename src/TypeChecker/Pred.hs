...
import TypeChecker.Kind (Kind (Kfun, Star))
import TypeChecker.PPrint (PPrint (..), nest, text, ($$), (<+>))
...
import TypeChecker.Unify (Match (..), Unify (..))
...
lift :: (MonadFail m) => ([Type] -> [Type] -> m a) -> Pred -> Pred -> m a
